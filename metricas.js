document.addEventListener('DOMContentLoaded', async () => {
 ...
 async function load(event){
  ...
  try{
   ...
   notice('Dados dos canais consultados.'+(summary.refreshing?' Atualizando dados salvos em segundo plano…':''));
   async function refreshCurrent(){if(current!==generation)return;if(document.hidden){setTimeout(refreshCurrent,60000);return;}try{summary=await liveSummary(start,end);if(current!==generation)return;await mergeShopee(summary,start,end);if(current!==generation)return;paint(comparison);el('updated').textContent='Consultado às '+new Date().toLocaleTimeString('pt-BR');notice('Dados atualizados.'+(summary.refreshing?' Atualizando dados salvos…':''));}catch(error){if(current===generation)notice('Últimos dados exibidos. '+error.message,'error');}if(current===generation)setTimeout(refreshCurrent,summary.refreshing?3000:60000);}
